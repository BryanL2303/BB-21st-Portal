...
const UserInformation = ({ accountType, appointment, userId, showForm, reLoad }) => {
	...
	useEffect(() => {
		setAccount(null)
		axios.post(`/api/account/${userId}/get_account_information`, { 'id': userId }, { withCredentials: true })
		.then(resp => {
	...
	function editAccount(e) {
		...
		if (submit) {
			axios.post('/api/account/' + account.id + '/edit_account', {
				...
				class1: account.account_type === "Officer" ? accountClass : e.target.elements['class_1']?.value,
	...
	function deleteAccount(e) {
		e.preventDefault()
		axios.post('/api/account/0/delete_account', { account_name: account.account_name }, { withCredentials: true })
		.then(resp => {
			if (resp.data != false) {
				// Deletion failed
		...
		<div className='user-information'>
			...
			{account != null &&
				<form className="edit-account-form" id='edit-account-form' onSubmit={editAccount} data-testid='user-information-form'>
					...
					{accountType == 'Admin' && <>
						<label htmlFor='password-input'>Hashed Password:</label>
						<input name={"password"} className='edit-field' defaultValue={account.password_digest} id='password-input' placeholder='Enter Password' />
					</>}
